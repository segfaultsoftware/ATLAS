...
RSpec.describe "Pawn", type: :model do
  ...
  describe "database constraints" do
    ...
    it "rejects null, untrimmed, empty required, and overlong names" do
...

      {
        ...
        invalid_values.each do |value|
          expect do
            Pawn.where(id: pawn.id).update_all(attribute => value)
    ...
    it "allows an empty or null last name" do
      expect do
        Pawn.where(id: pawn.id).update_all(last_name: "")
        Pawn.where(id: pawn.id).update_all(last_name: nil)
      end.not_to raise_error
    end

    it "rejects null birth turns and nonpositive or null stats" do
      expect do
        Pawn.where(id: pawn.id).update_all(born_on_turn: nil)
      end.to raise_error(ActiveRecord::StatementInvalid)
    ...
    it "rejects an absent Game" do
      expect do
        Pawn.where(id: pawn.id).update_all(game_id: -1)
      end.to raise_error(ActiveRecord::StatementInvalid)
    end
  end
end
